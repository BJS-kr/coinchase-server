...
// run test with "go test ./server_test -v -race"
...
func TestMain(m *testing.M) {
	...
	httpServer := http_server.NewServer(10, 10)
...
func TestWorkerPullOut(t *testing.T) {
	...
	t.Run("최대 수의 유저(워커의 갯수 만큼) 로그인", func(t *testing.T) {
		...
		for _, cl := range clientListeners {
			// 이 시점에서 이미 worker들이 dial중이기 때문에 accept가 곧장 진행된다.
			conn, err := cl.Listener.AcceptTCP()

			if err != nil {
				t.Fatalf("failed to accept TCP: %s", err)
			}
...
func TestPlay(t *testing.T) {
	...
	t.Run("유저의 랜덤 위치 이동", func(t *testing.T) {
		...
		for _, testClient := range clientListeners {
			...
			waiter.Add(1)
			conn, err := net.Dial("tcp", fmt.Sprintf("127.0.0.1:%d", testClient.WorkerPort))
			...
			go func() {
				...
				for {
					...
					default:
						...
						if newUserPosition.X < 0 {
							newUserPosition.X = 0
						}
						if newUserPosition.Y < 0 {
							newUserPosition.Y = 0
						}
