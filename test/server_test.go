...
// 또 한, 이 구현은 테스트에 표현되지 않은(표현하기 힘든) 부분이 많다.
// 예를 들어, 일정 간격마다 워커풀을 검사해 유효하지 않은 워커를 종료시키고, 되살리는 등이다.
// 말 그대로 게임로직에서 에러가 일어나야 확인이 가능한 형태로 구성되어있다. 물론 일부러 에러가 나도록 구현되어있지 않다.
			...
			go func() {
				...
				for !timeoutReached {
					...
					newUserPosition := &protodef.Position{
						X: userPosition.X + game_map.GenerateRandomDirection(),
						Y: userPosition.Y + game_map.GenerateRandomDirection(),
					}
